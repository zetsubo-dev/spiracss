...
**`:global(...)` の注意:** リンターは global-only セレクタを除外し、ローカル部分が残る場合のみ検証します。  
右端ターゲットが global になるケース（例: `.block :global(.foo)` / `.block:is(:global(.foo))`）は、そのセレクタ自体を out-of-scope として配置チェックをスキップします。  
