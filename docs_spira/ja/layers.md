...
- `.u-*` で始まるユーティリティクラスは Block / Element（class モード時の Modifier）とは別カテゴリとして扱います。**ユーティリティ単独を許容するため、`allowExternalPrefixes` に `u-` を追加する運用を標準**とします。2 語 / 1 語といった厳密な制約は構造用クラスだけに課し、ユーティリティは補助レイヤとして扱います。
