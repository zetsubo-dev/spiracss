...
**global/config.scss**: ブレークポイントや共通のベース値など、サイト全体で共有する「設定値」を管理します。

**variables.scss**: カラーパレット・タイポグラフィ・スペーシングなど、デザイン設計上の値を集中管理します。

> **運用ルール**: 新しい設計値が必要になったら `global/config.scss` または `variables.scss` に追加したうえで使用し、直接ハードコードはしません。

### global/function.scss / global/mixin.scss

**global/function.scss**: 複数のコンポーネントから利用される数値変換ユーティリティ関数をまとめます。
...
**global/mixin.scss**: ブレークポイントやレイアウトに関する汎用 Mixin を定義します。
...
**base.scss**: 見出し・段落などの基本要素スタイルを定義します。  
**print.scss**: 印刷用の基本スタイルを定義します。  
**reset.scss**: ブラウザ差分を均すリセットを定義します。

### global/index.scss

**global/index.scss**: `@use "@styles/partials/global"` の入口として、`global/` 配下をまとめて読み込むためのファイルです。
...
- `.u-*` で始まるユーティリティクラスは Block / Element（class モード時の Modifier）とは別カテゴリとして扱います。**ユーティリティ単独を許容するため、`stylelint.base.external.prefixes` に `u-` を追加する運用を標準**とします。2 語 / 1 語といった厳密な制約は構造用クラスだけに課し、ユーティリティは補助レイヤとして扱います。
...
`@keyframes` は CSS 仕様上グローバルです。再利用するアニメーションは `styles/partials/keyframes.scss` に集約し、`kf-` などの prefix を付けます。  
`@keyframes` はルート直下のみ（@media/@layer 内は不可）。  
このファイルは keyframes 専用にするか、`@keyframes` を末尾にまとめます（末尾はコメント／空行のみ）。
