    ...
    reject: [
      {
        ...
        message:
          'Write modifier classes inside the Block using `&.<modifier>`. Example: `.block { &.-primary { ... } }`. Do not use `.block.-primary` or `.-primary` at top level. (spiracss/class-structure)'
      },
      {
        code: '.block.-modifier {}',
      ...
      },
      {
        ...
        message:
          'Only modifier classes may be appended to `&`. Found `modifier`. Use `&.<modifier>`. Example: `&.-primary`. If not a modifier, move it to its own selector. (spiracss/class-structure)'
      }
    ]
  })
})
