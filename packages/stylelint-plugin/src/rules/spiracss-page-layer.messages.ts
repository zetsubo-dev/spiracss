...
export const messages = createRuleMessages(ruleName, {
  missingComponentLink: (selector: string) =>
    ...
    `Add a link comment as the first node in the rule (e.g., ${formatCode('// @components/...')}).`,
  ...
  selectorParseFailed: (...args: RuleMessageArgs) => formatSelectorParseFailed(args[0] as string | undefined)
})
