...
const formatAliasList = (aliases: string[]): string => formatConfigList(aliases.map((alias) => `@${alias}/`))

const formatAliasHint = (aliases: string[]): string => ` (current: ${formatAliasList(aliases)})`
...
export const messages = createRuleMessages(ruleName, {
  ...
  misplacedParentRel: (aliases: string[]) =>
    ...
    `Move it above the root Block as the first line (e.g., ${formatCode('// @rel/...')}). ` +
    `Use a configured alias from ${formatCode('aliasRoots')}${formatAliasHint(aliases)}.`,
  rootBlockNotFirst: () =>
  ...
  missingChildRel: (aliases: string[]) =>
    ...
    `using ${formatCode(
      'aliasRoots'
    )}${formatAliasHint(aliases)} as the first line inside each direct child rule (${formatCode('> .child')}). ` +
    `Example: ${formatCode('> .child { // @rel/child.scss }')}.`,
  notFound: (target: string) =>
    `Link target not found: ${formatCode(target)}. Fix the path or ${formatCode('aliasRoots')}.`,
  childMismatch: (child: string, expectedFiles: string[]) =>
    `Link comment must include ${formatConfigList(expectedFiles)} for direct child ${formatCode(
      `.${child}`
